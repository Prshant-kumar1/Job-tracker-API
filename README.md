...
A backend API for tracking internship/job applications, updating their status, viewing dashboard analytics, and getting AI-generated next-step suggestions — built to be portfolio-ready, not a toy CRUD demo.
...
# 4. Copy the env file (defaults work fine for local dev)
cp .env.example .env
...
> **Note:** `requirements.txt` pins `bcrypt==4.0.1` alongside `passlib==1.7.4`. Newer bcrypt releases (4.1+) removed an internal attribute that this version of passlib checks for, which causes password hashing to fail with a `ValueError` about 72-byte limits. Keep this pin unless you upgrade passlib too.
